...
class Observation:
    ...
    def __init__(self, box_path: str,snap_num: int, save_dir: str, run_ID: str, emin: float, emax: float, emin_for_EW_values:float, emax_for_EW_values:float, emin_for_Lx_tot:float, emax_for_Lx_tot:float, overwrite = False, redshift = "from_box", h = "from_box", test_align = False, enable_parallelism = False,):
        ...
        self.emin_for_Lx_tot = emin_for_Lx_tot
        self.emax_for_Lx_tot = emax_for_Lx_tot
    ...
    def add_instrument(self, Name: str, exp_time: float, ID = None, reblock=1, aim_shift=None, chip_width=None):
        ...
        try:
            ...
            self._logger.error("Could Not Find instrument in defaults list")  
    ...
    def MakePhotons(self, area = (2.5, "m**2"), nbins = 6000,  metals = None, photons_emin = 0, photons_emax = 10, thermal_model = "apec", nH_val = 0.018, absorb_model="tbabs", sphere_R500s = 5, const_metals = False, thermal_broad=True, orient_vec = (0,0,1), north_vector=None, generator_field = None, photon_sample_exp = 1200, only_profiles = False, make_profiles = True, overwrite = False):
        '''
        ...
        if self.enable_parallelism:
            yt.enable_parallelism()
        import pyxsim
        self._photons_emin = min(photons_emin, self.emin)
        self._photons_emax = max(photons_emax, self.emax)
        ...
        yt.add_xray_emissivity_field(self.ds, self.emin_for_Lx_tot, self.emax_for_Lx_tot, table_type="apec", metallicity = (self.generator_field, "metallicity") , redshift=self.redshift, cosmology=self.ds.cosmology, data_dir="./CODE/instr_files/")
        ...
        for i,halo in enumerate(self.active_halos):
            ...
            ''' Produce event files for the projected photons convolved with the instrumental response for each instrument. Also write the spectrum to file'''
            
            
    def ObservePhotons(self, image_energies = None, instr_bkgnd = True, foreground = False, ptsrc_bkgnd = True, no_dither = False, overwrite = False):
        '''
        ...
        for i,halo in enumerate(self.active_halos):
            ...
            for instrument in self.active_instruments:
                ...
                fig, ax = soxs.plot_spectrum(f"{evts_path}/{idx_instr_tag}_evt.pha", xmin=0.8 * self.emin, xmax=1.05*self.emax, lw = 0.5, color = "black" )
                ...
                plt.legend()  
                plt.savefig(f"{evts_path}/{idx_instr_tag}_spectrum.png")            
                ...
                if self._image_energies != None:   
                    for energy_dict in self._image_energies:
                        ...
                        fig, ax = soxs.plot_image(f"{evts_path}/{idx_instr_tag}_img_{energy_dict['name']}.fits", stretch='log', cmap='cubehelix', width = instrument['image_width'])
                ...
                if os.path.exists(f"{evts_path}/{idx_instr_tag}_evt.fits"):
                    ...
                    if self._image_energies == None:
                        ...
                        with astropy.io.fits.open(f"{evts_path}/{idx_instr_tag}_img.fits") as hdul:
                            ...
                            ax.scatter(center[0],center[1], c = "yellow", marker = "+", s = 1000000, linewidths= 0.5)
                            instrument_spec = instrument_registry[instrument_name]
                            chip_width = np.array(instrument_spec["chips"])[1][[3,4]].astype(np.float)
    ...
    def _yT_profiles(self):
        ...
        self._logger.info(f"Total Lx in R500 being calculated with xray_luminosity_{self.emin_for_Lx_tot}_{self.emax_for_Lx_tot}_keV field")
        lumin_field_for_Lx_tot = str(f"xray_luminosity_{self.emin_for_Lx_tot}_{self.emax_for_Lx_tot}_keV")
        ...
        rp_data.append(  {"Name":f"total_Lx_in_R500_{self.emin_for_Lx_tot}_{self.emax_for_Lx_tot}_keV", "value": total_Lx_in_R500.to_astropy()  }) 
        self._logger.info("yT Data Successfully Taken")
        np.save(f"{yt_data_path}/{self._idx_tag}_yt_data_ver3.npy",rp_data)  
