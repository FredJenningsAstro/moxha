...
class HaloSample():
    ...
    def plot_Xray_mass_bias(self, plot_running_median=True, median_kernel_size=3,  median_alpha = 0.5,  median_lims = (0,-1), plot_running_percentiles = (16,84), plot_legend = False):
        print("MEDIANS NOT CORRECT---NEED TO ADD IN ROMEEL'S CODE")
        sys.exit()
        xmin_plt_lim = 8e11 
        xmax_plt_lim = 1e15
    ...
    def plot_Xray_mass_vs_EW_mass(self, plot_running_median = True, error_alpha = 0.5, scatter_linewidth = 0.9, median_kernel_size = 5, median_alpha = 0.5, median_lims = (0,-1), xmin_plt_lim = 3e12,xmax_plt_lim = 9.9e14, plot_running_percentiles = (16,84), plot_legend = False,  plot_ODR_median = False, bad_ODR_idxs = []):
        ...
        if plot_running_median:
            ...
            x_data = x_data[sort_idxs]
            y_data = y_data[sort_idxs]
            x_data, running_median, running_perc1, running_perc2 = self.runningmedian(x_data, y_data/x_data, bins = -median_kernel_size, erange = (16,84))
            print("bin x points", x_data)
            running_perc1 = running_median - running_perc1
            running_perc2 = running_median + running_perc2
            # running_median = median_filter(y_data/x_data, size = median_kernel_size)
            # running_perc1 = percentile_filter(y_data/x_data,percentile = min(plot_running_percentiles), size = median_kernel_size)
            # running_perc2 = percentile_filter(y_data/x_data,percentile = max(plot_running_percentiles), size = median_kernel_size)
            plt.plot(x_data, (running_median), color = "black", lw = 3, ls = "solid", alpha = median_alpha)  
            plt.sca(frame1)
            plt.plot(x_data, (running_median*x_data), color = "black", lw = 3, ls = "solid", alpha = median_alpha)    
            if plot_running_percentiles != False:
                    plt.sca(frame2)
                    plt.fill_between(x_data, y1 = running_perc1, y2 = running_perc2, color = "grey", alpha = 0.5*median_alpha)   
                    plt.sca(frame1)
                    plt.fill_between(x_data, y1 = (running_perc1*x_data), y2 = (running_perc2*x_data), color = "grey", alpha = 0.5*median_alpha) 
            ...
            if plot_ODR_median:
                ...
                # running_perc1 = percentile_filter(y_data/x_data,percentile = min(plot_running_percentiles), size = median_kernel_size)
                # running_perc2 = percentile_filter(y_data/x_data,percentile = max(plot_running_percentiles), size = median_kernel_size)
                plt.plot(x_data, (running_median), color = "red", lw = 3, ls = "dashed", alpha = median_alpha)  
                plt.sca(frame1)
                plt.plot(x_data, (running_median*x_data), color = "red", lw = 3, ls = "dashed", alpha = median_alpha)    
                if plot_running_percentiles != False:
                        plt.sca(frame2)
                        plt.fill_between(x_data, y1 = running_perc1, y2 = running_perc2, color = "maroon", alpha = 0.2*median_alpha)   
                        plt.sca(frame1)
                        plt.fill_between(x_data, y1 = (running_perc1*x_data), y2 = (running_perc2*x_data), color = "maroon", alpha = 0.2*median_alpha)  
    ...
    def plot_loglog_scaling(self, x_property_key, y_property_key, xerr_key, yerr_key, mass_key, xlabel, ylabel, Ez_power, self_sim_scaling_power, xmin_plt_lim , xmax_plt_lim, ymin_plt_lim, ymax_plt_lim, save_name, best_fit_min_mass = 0, best_fit_max_mass = 1e50, write_norm = False, cscale = None, cscale_scale = "log", data_labels = True,  plot_self_similar_best_fit = False, plot_best_fit=True, external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed"}, best_fit_bins=8, legend_fontsize = 16 ):
        ...
        largest_bias = max(1-np.asarray([halo[mass_key]/halo["M500_truth"] for halo in halo_samples]))
        smallest_bias = min(1-np.asarray([halo[mass_key]/halo["M500_truth"] for halo in halo_samples]))
        cmap = matplotlib.cm.viridis
        norm = matplotlib.colors.Normalize(vmin=smallest_bias, vmax=largest_bias)        
        if mass_key == "M500_truth": norm = matplotlib.colors.Normalize(vmin=0, vmax=1)
        ...
        for halo in halo_samples:
            ...
            marker_color = cmap(norm(1-(halo[mass_key]/halo["M500_truth"])))
            marker = "o"
            if mass_key == "M500_truth": marker_color = cmap(norm(0.5))
            # print(marker_color)
            ...
            if xerr_key == None and yerr_key == None:
                plt.scatter(halo[x_property_key],    Ez_correction * halo[y_property_key], facecolors=marker_color, edgecolors=marker_color, marker = marker, s = 100, linewidths = 0.6, alpha = our_data_alpha) 
                pass
            elif xerr_key == None:
                plt.errorbar(halo[x_property_key].value, Ez_correction * halo[y_property_key].value,  yerr = Ez_correction * yerr, capsize = 8, fmt = 'None', color = marker_color, elinewidth = 0.6, alpha = our_data_alpha)                    
            elif yerr_key == None:
                plt.errorbar(halo[x_property_key].value, Ez_correction * halo[y_property_key].value, xerr = xerr, capsize = 8, fmt = 'None', color = marker_color, elinewidth = 0.6, alpha = our_data_alpha)
            else:
                plt.errorbar(halo[x_property_key].value, Ez_correction * halo[y_property_key].value, xerr = xerr, yerr = Ez_correction * yerr, capsize = 8, fmt = 'None', color = marker_color, elinewidth = 0.6, alpha = our_data_alpha)
            if halo['label'] in plotted_labels or data_labels == False:
                plt.scatter(halo[x_property_key],    Ez_correction * halo[y_property_key], facecolors=marker_color, edgecolors=marker_color, marker = marker, s = 100, linewidths = 0.9, alpha = our_data_alpha)
            else:
                plt.scatter(halo[x_property_key],    Ez_correction * halo[y_property_key], facecolors=marker_color, edgecolors=marker_color, marker = marker, s = 100, linewidths = 0.9, label = halo['label'], alpha = our_data_alpha)           
        ...
        ### Prettyplot mean bins over whole mass range
        if best_fit_bins != False:
            ...
            while True:
                ...
                plt.plot(10**logx_data, 10**logy_data, color = "mediumvioletred", lw = 3, alpha = 1,  zorder = 20000)
                break


        ### Bin the data for the BCES fit
        ...
        if best_fit_bins != False:
            ...
            bces_logx_data, bces_logy_data, bces_err_logx, bces_err_logy = self._return_binned_means( mass_key, best_fit_min_mass, best_fit_max_mass, mass_data, x_data, y_data, x_data_err, y_data_err, best_fit_bins)
            cov = np.zeros_like(bces_logx_data)
            ...
            bces_err_logx = np.array(err_logx[sort_idxs])
            bces_err_logy = np.array(err_logy[sort_idxs])
            if best_fit_min_mass != None and best_fit_min_mass > 0: print(f"BCES fit will include halos with {mass_key} > 10$^{{{np.log10(best_fit_min_mass)}}}$")
            ...
            bces_err_logx = np.array(bces_err_logx[mass_range_idxs])
            bces_err_logy = np.array(bces_err_logy[mass_range_idxs])
            min_length = 10
            if len(bces_x_data) < min_length:
                print("Less than required number for bces")
                return [None,], None, len(bces_x_data)
            
            bces_logx_data = np.log10(bces_x_data)
            bces_logy_data = np.log10(bces_y_data)
        ...
        if plot_best_fit:
            nboot=100_000
            bcesMethod = 3
            print(f"BCES will see the data pairs {list(zip(bces_logx_data,bces_logy_data))}")
            print("max difference between x and y = ", max([abs(x[0]-x[1]) for x in list(zip(bces_logx_data,bces_logy_data))] ))
            a,b,erra,errb,covab=BCES.bcesp(y1= bces_logx_data,y1err = bces_err_logx,y2 = bces_logy_data,y2err=bces_err_logy,cerr=cov,nsim=nboot)  
            ...
            try:
                lcb,ucb,xcb=nmmn.stats.confbandnl(bces_logx_data,bces_logy_data,nmmnfunc,fitm,covm,2,confidence_interval,np.linspace(bces_logx_data.min(), bces_logx_data.max(), num = 1000))
            except Exception as e:
                print(e)
                print("covm", covm)
        ...
        if plot_best_fit:
            ...
            if best_fit_min_mass != None and best_fit_min_mass>0:
                ...
                grad_string = f"Best Fit ($\mathtt{{M}}_\mathtt{{500,{mass_tag}}}$ > 10$^{{{np.log10(best_fit_min_mass)}}} M_\odot$), \n {'': <7} m(Y|X) = {'%.2f' % a[0]} $\pm$ {'%.2f' % erra[0]} \n {'': <7} m(Ogl) = {'%.2f' % a[3]} $\pm$ {'%.2f' % erra[3]}"
                print(grad_string)
                ...
                if write_norm:
                    grad_string = f"Best Fit m(Y|X) = {'%.2f' % a[0]} $\pm$ {'%.2f' % erra[0]} \n {'': <14} b= {'%.2f' % b[0]} $\pm$ {'%.2f' % errb[0]} \n {'': <7} m(Ogl) = {'%.2f' % a[3]} $\pm$ {'%.2f' % erra[3]} \n {'': <14} b= {'%.2f' % b[3]} $\pm$ {'%.2f' % errb[3]} "
                else:
                    grad_string = f"Best Fit m(Y|X) = {'%.2f' % a[0]} $\pm$ {'%.2f' % erra[0]} \n {'': <7} m(Ogl) = {'%.2f' % a[3]} $\pm$ {'%.2f' % erra[3]}"
                if bcesMethod == 0: print("Best fit line will be Y|X")
                if bcesMethod == 3: print("Best fit line will be Orthogonal")
        ...
        plt.xlabel(xlabel)
        if "[$\mathtt{M}_\odot$]" not in ylabel:
    ...
    def plot_M500_vs_weighted_kT_Xray_scaling(self, cscale = None, mass_key=None,  cscale_scale = "log", ymin_plt_lim = 3e12, ymax_plt_lim = 1e15, xmin_plt_lim = 0.1, xmax_plt_lim = 10,data_labels = True,  plot_self_similar_best_fit=False, plot_best_fit=True, external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed"}, best_fit_min_mass = 0,    legend_fontsize = 20 ):
        ...
        if mass_key == None: mass_key =  "Xray_HSE_M500"
        if hubble_correction:
            ylabel = r"E(z)$\mathtt{M}_\mathtt{500,X}$[$\mathtt{M}_\odot$]"
        if not hubble_correction:
            ylabel = r"$\mathtt{M500}_\mathtt{X}}$[$\mathtt{M}_\odot$]$"
        if cscale == None:
            if hubble_correction:
    ...
    def plot_M500_vs_kTR500_LW_scaling(self, cscale = None, mass_key=None,  cscale_scale = "log", ymin_plt_lim = 3e12, ymax_plt_lim = 1e15, xmin_plt_lim = 0.1, xmax_plt_lim = 10,data_labels = True,  plot_self_similar_best_fit=False, plot_best_fit=True, external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed"}, best_fit_min_mass = 0,    legend_fontsize = 20 ):
        ...
        if mass_key == None: mass_key =  "LW_HSE_M500"
        if hubble_correction:
            ylabel = r"E(z)$\mathtt{M}_\mathtt{500,LW}$[$\mathtt{M}_\odot$]"
        if not hubble_correction:
    ...
    def plot_M500_vs_kTR500_EW_scaling(self, cscale = None, mass_key=None,  cscale_scale = "log", ymin_plt_lim = 3e12, ymax_plt_lim = 1e15, xmin_plt_lim = 0.1, xmax_plt_lim = 10,data_labels = True,  plot_self_similar_best_fit=False, plot_best_fit=True, external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed"}, best_fit_min_mass = 0,    legend_fontsize = 20 ):
        ...
        if mass_key == None: mass_key =  "EW_HSE_M500"
        if hubble_correction:
            ylabel = r"E(z)$\mathtt{M}_\mathtt{500,EW}$[$\mathtt{M}_\odot$]"
        if not hubble_correction:
            ylabel = r"$\mathtt{M500}_\mathtt{EW}$[$\mathtt{M}_\odot$]$"
        if cscale == None:
            if hubble_correction:
    ...
    def plot_M500_vs_kTR500_MW_scaling(self, cscale = None, mass_key=None,  cscale_scale = "log", ymin_plt_lim = 3e12, ymax_plt_lim = 1e15, xmin_plt_lim = 0.1, xmax_plt_lim = 10,data_labels = True,  plot_self_similar_best_fit=False, plot_best_fit=True, external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed"}, best_fit_min_mass = 0,    legend_fontsize = 20 ):
        ...
        if mass_key == None: mass_key =  "MW_HSE_M500"
        if hubble_correction:
            ylabel = r"E(z)$\mathtt{M}_\mathtt{500,MW}$[$\mathtt{M}_\odot$]"
        if not hubble_correction:
    ...
    def plot_M500_vs_weighted_kT_MW_scaling(self, cscale = None, mass_key=None,  cscale_scale = "log", ymin_plt_lim = 3e12, ymax_plt_lim = 1e15, xmin_plt_lim = 0.1, xmax_plt_lim = 10,data_labels = True,  plot_self_similar_best_fit=False, plot_best_fit=True, external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed"}, best_fit_min_mass = 0,    legend_fontsize = 20 ):
        ...
        if hubble_correction:
            ylabel = r"E(z)$\mathtt{M}_\mathtt{500,MW}$[$\mathtt{M}_\odot$]"
        if not hubble_correction:
            ylabel = r"$\mathtt{M500}_\mathtt{MW}}$[$\mathtt{M}_\odot$]"
        if cscale == None:
            if hubble_correction:
    ...
    def plot_S_EW_at_EW_R500_scaling(self, cscale = None, mass_key=None,  cscale_scale = "log", xmin_plt_lim = 3e12, xmax_plt_lim = 1e15, ymin_plt_lim = 100, ymax_plt_lim = 1000,  plot_self_similar_best_fit=False, plot_best_fit=True, data_labels = True,  external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed","label":"best fit"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed", "label":"Self-Similar best fit"}, best_fit_min_mass = 0,    legend_fontsize = 20  ):
        Ez_power = 2/3
        self_sim_scaling_power = 2/3
        ...
        if mass_key == None: mass_key =  "EW_HSE_M500"
        if hubble_correction:
            ylabel = r"E(z)$^{\mathtt{2/3}}$S$_\mathtt{EW}(\mathtt{R}_\mathtt{500,EW})$[keV cm$^2$]"
        if not hubble_correction:
            ylabel = r"$\mathtt{S}_\mathtt{EW}(\mathtt{R}_\mathtt{500,EW})$[keV/cm$^2$]"
        if cscale == None:
            if hubble_correction:
                save_name = f"S_EW_at_EW_R500_Ez_corrected"
            else:
                save_name = f"S_EW_at_EW_R500"
        else:
            if hubble_correction:
                save_name = f"S_EW_at_EW_R500_Ez_corrected_vs_{cscale}"
            else:
                save_name = f"S_EW_at_EW_R500_vs_{cscale}"   
        self.plot_loglog_scaling(x_property_key, y_property_key, xerr_key, yerr_key, mass_key, xlabel, ylabel, Ez_power, self_sim_scaling_power, xmin_plt_lim , xmax_plt_lim, ymin_plt_lim, ymax_plt_lim, save_name, cscale = cscale, cscale_scale = cscale_scale, data_labels = data_labels,  plot_self_similar_best_fit = plot_self_similar_best_fit, plot_best_fit=plot_best_fit, external_dsets = external_dsets, hubble_correction = hubble_correction, best_fit_kwargs=best_fit_kwargs, self_similar_best_fit_kwargs=self_similar_best_fit_kwargs, best_fit_min_mass = best_fit_min_mass,   legend_fontsize = legend_fontsize )    
    ...
    def plot_Xray_Lx_vs_true_Lx(self, emin, emax, cscale = None, mass_key=None,  cscale_scale = "log", xmin_plt_lim = 3e12, xmax_plt_lim = 1e15, ymin_plt_lim = 100, ymax_plt_lim = 600,  plot_self_similar_best_fit=False, plot_best_fit=True, data_labels = True,  external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed","label":"best fit"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed", "label":"Self-Similar best fit"}, best_fit_min_mass = 0,    legend_fontsize = 20  ):
        Ez_power = 0
        hubble_correction = False
        self_sim_scaling_power = 0
        
        Xray_total_lumin_emin_RF = 0.5
        Xray_total_lumin_emax_RF = 2.0
        
        Xray_true_total_lumin_emin_RF = 0.5
        Xray_true_total_lumin_emax_RF = 2    
        
        
        x_property_key = f"Lx_in_R500_truth_{Xray_true_total_lumin_emin_RF}_{Xray_true_total_lumin_emax_RF}_keV"
        y_property_key = f"Xray_L_in_Xray_HSE_R500_{Xray_total_lumin_emin_RF}-{Xray_total_lumin_emax_RF}_RF_keV"
        xerr_key = None
        yerr_key = f"Xray_L_in_Xray_HSE_R500_spread_{Xray_total_lumin_emin_RF}-{Xray_total_lumin_emax_RF}_RF_keV"
        xlabel = f"$L_{{true}} ^\mathrm{{{Xray_true_total_lumin_emin_RF}-{Xray_true_total_lumin_emax_RF}}}$[erg/s]"
        if mass_key == None: mass_key =  "Xray_HSE_M500"
        if not hubble_correction:
            ylabel = f"$L_X ^\mathrm{{{Xray_total_lumin_emin_RF}-{Xray_total_lumin_emax_RF}}}$[erg/s]"
        if cscale == None:
            if hubble_correction:
                save_name = f"L_Xray_Ez_corrected"
            else:
                save_name = f"L_Xray_vs_L_true"

        self.plot_loglog_scaling(x_property_key, y_property_key, xerr_key, yerr_key, mass_key, xlabel, ylabel, Ez_power, self_sim_scaling_power, xmin_plt_lim , xmax_plt_lim, ymin_plt_lim, ymax_plt_lim, save_name, cscale = cscale, cscale_scale = cscale_scale, data_labels = data_labels,  plot_self_similar_best_fit = plot_self_similar_best_fit, plot_best_fit=plot_best_fit, external_dsets = external_dsets, hubble_correction = hubble_correction, write_norm=True, best_fit_kwargs=best_fit_kwargs, self_similar_best_fit_kwargs=self_similar_best_fit_kwargs, best_fit_min_mass = best_fit_min_mass, legend_fontsize = legend_fontsize )
    ...
    def plot_true_Lx_scaling(self,emin, emax, cscale = None, mass_key=None,  cscale_scale = "log", xmin_plt_lim = 3e12, xmax_plt_lim = 1e15, ymin_plt_lim = 100, ymax_plt_lim = 600,   plot_self_similar_best_fit=False, plot_best_fit=True, data_labels = True,  external_dsets = [], hubble_correction = True, best_fit_kwargs={"color":"grey", "lw":3, "ls":"dashed","label":"best fit"}, self_similar_best_fit_kwargs={"color":"blue", "lw":3, "ls":"dashed", "label":"Self-Similar best fit"}, best_fit_min_mass = 0,    legend_fontsize = 20  ):
        ...
        if hubble_correction:
            ylabel = r"E(z)$^{\mathtt{-7/3}}$L" + f"$^\mathrm{{{Xray_total_lumin_emin_RF}-{Xray_total_lumin_emax_RF}}}$[erg/s]"
        if not hubble_correction:
    ...
    def runningmedian(self, x,y,xlolim=-1.e20,ylolim=-1.e20,erange=None,bins=10,stat='median'):
        ...
        if bins < 0:	# bins<0 sets bins such that there are equal numbers per bin
            ...
            bin_means, bin_edges, binnumber = scipy.stats.binned_statistic(xp,yp,bins=bin_edges,statistic=stat)
        else:
            bin_means, bin_edges, binnumber = scipy.stats.binned_statistic(xp,yp,bins=bins,statistic=stat)
        ...
        for i in range(0,len(bin_edges[:-1])):
            ...
            if erange is not None:
                ylo.append(np.percentile(10**ysub,erange[0]))
                yhi.append(np.percentile(10**ysub,erange[1]))
        ...
        if erange == None:
            ...
            ysiglo = np.log10(ymean)-np.log10(ylo)
            ysighi = np.log10(yhi)-np.log10(ymean)
    ...
    def generate_2D_ks_test_table(self, keys,  min_mass = 0, significance = 0.05, max_mass = 1e30, mass_key = None, ks_kwargs = {}, min_num = 10):
        ...
        keys2 = np.array([key for key in keys])
        if "Xray_ODR_HSE_M500" in keys2: keys2[keys2=="Xray_ODR_HSE_M500"] = "XrayODR_HSE_M500"
        print("key2", keys2)
        table_string += " & " + " & ".join([key.split('_')[0] for key in list(reversed(keys2))]) + r"\\ \hline\hline" + "\n"
        for key1 in keys:
            if key1 == "Xray_ODR_HSE_M500":
                table_string += "Xray_ODR" + " & "
            else:
            ...
            for key2 in list(reversed(keys)):
                ...
                if key1 == key2 or keys.index(key1) > keys.index(key2):
                    ...
                    print(key1, key2, f"({round(ks_stats[0],3)}, {'{:.2e}'.format(ks_stats[1])})")
    ...
    def plot_bias_hist_panel(self,key_pairs, min_mass = 0, max_mass = 1e30, ylims = (0,100), xlims = None, mass_key = None, normalise = True, bins = 20, plot_multiple_medians = True, legend_fontsize=20, colors = None, bad_ODR_idxs = []): 
        ...
        for j,key_pair in enumerate(key_pairs):
            ...
            for i,keyi in enumerate(key_pair):
                ...
                q = np.diff(answer)
                quant_label_template = "${0:.2f}_{{-{1:.2f}}}^{{+{2:.2f}}}$"
                quant_label = quant_label_template.format(answer[1], q[0], q[1])
                label = f"{keyi.split('_HSE')[0]} ({quant_label})"
                sigma = (q[1]+q[0])/2
                se_mean = sigma/np.sqrt(len(X1))
                se_median = 1.2533 * se_mean
                print(f"{keyi} with {len(X1)} Samples, Standard Errors on Median: {'%.2f' % answer[1]} +/- {'%.4f' % se_median}")
                frame.hist(X1, bins = bins, density=normalise,  lw=3, ec=colors[keyi], histtype = "step", alpha=0.5, label = label)
                if plot_multiple_medians:
                    frame.vlines(x = answer[1], ymin = 0, ymax = 200, color = colors[keyi], ls = "dashed")
            if normalise:
            ...
            if j == 0 and min_mass > 0:
                ...
                frame.legend(fontsize=legend_fontsize, framealpha = 1)    
    ...
    def _loglog_scaling_bces(self, x_property_key, y_property_key, xerr_key, yerr_key, Ez_power, self_sim_scaling_power,save_name,min_length, mass_key, plot_loc = None, bces_methods = [0,3], best_fit_bins = 8, hubble_correction = True,nboot=10000,  best_fit_min_mass = 0, best_fit_max_mass = 1e30, natural_mass_key = None):
        import bces.bces as BCES
        import nmmn.stats
        ...
        bces_logx_data, bces_logy_data, bces_err_logx, bces_err_logy = self._return_binned_means( mass_key, best_fit_min_mass, best_fit_max_mass, mass_data, x_data, y_data, x_data_err, y_data_err, best_fit_bins=8)

        
        cov = np.zeros_like(bces_logx_data)
        
        #print("bces_x_data", bces_x_data)
        print(f"BCES with {len(bces_logx_data)} bins in the x-variable and with {nboot} bootstrap samples")
        a,b,erra,errb,covab=BCES.bcesp(y1= bces_logx_data,y1err = bces_err_logx,y2 = bces_logy_data,y2err=bces_err_logy,cerr=cov,nsim=nboot)   
        if plot_loc != None:
            handles, labels = plt.gca().get_legend_handles_labels()
            by_label = dict(zip(labels, handles))
            
            plt.xlabel(x_property_key)
            plt.ylabel(y_property_key)
            # plt.xlim(left = xmin_plt_lim, right = xmax_plt_lim)
            # plt.ylim(bottom = ymin_plt_lim, top = ymax_plt_lim)
            plt.yscale("log")
            plt.xscale('log')
            os.makedirs(f"{self.save_dir}/{plot_loc}/", exist_ok = True)
            plt.scatter(10**bces_logx_data, 10**bces_logy_data, color = "navy", s = 600, marker = "1", zorder = 20000)
            plt.errorbar(10**bces_logx_data, 10**bces_logy_data, xerr = (10**bces_logx_data) * np.log(10)* bces_err_logx, yerr = (10**bces_logy_data) * np.log(10)* bces_err_logy, capsize = 5, fmt = 'None', color = "navy", elinewidth = 0.3, zorder = 20000)
            # plt.plot(10**bces_logx_data, 10**bces_logy_data, color = "navy", lw = 2, alpha = 0.6,  zorder = 20000)
            for bcesMethod in bces_methods:
                plt.plot([10**min(bces_logx_data), 10**max(bces_logx_data)],  (10**b[bcesMethod]) * (np.array([10**min(bces_logx_data), 10**max(bces_logx_data)])**(a[bcesMethod])),  label = f"Best Fit ($\mathtt{{M}}_\mathtt{{500,{mass_tag}}}$ > 10$^{{{np.log10(best_fit_min_mass)}}}$), m = {round(a[bcesMethod],2)}" )  
            plt.legend(by_label.values(), by_label.keys(), fontsize = 30)
            plt.savefig(f"{self.save_dir}/{plot_loc}/{y_property_key}_{x_property_key}_gtr_{best_fit_min_mass}.png", bbox_inches='tight')
            plt.close()     
        return a, b, erra, len(bces_logx_data)
    ...
    def generate_scaling_table(self,emin, emax, yT_emin, yT_emax, bces_methods, mass_key = None, min_masses= [None,], min_length = 5, flagged_length = 10, nboot = 100000):
        ...
        for scaling_dict in scaling_dicts:
            ...
            for min_mass in min_masses:
                ...
                m,b, merr, num_data = self._loglog_scaling_bces(**scaling_dict, nboot = nboot, mass_key = mass_key, best_fit_min_mass = min_mass, best_fit_max_mass = 1e30,  hubble_correction = True, min_length = min_length, plot_loc = "scaling_table", bces_methods = bces_methods)
    ...
    def mass_evolution_of_scaling_func_vs_min_mass(self,emin, emax, yT_emin, yT_emax, bces_methods, mass_key=None, min_masses= [None,], jump_size = 5, min_length = 10, nboot=10000):
        ...
        EW_dicts = [weighted_S_EW_vs_M500_EW_dict,] #[M500_EW_vs_weighted_kT_EW_dict, weighted_kT_EW_vs_M500_EW_dict, kT_EW_at_EW_HSE_R500_vs_M500_EW_dict, weighted_S_EW_vs_M500_EW_dict, M500_EW_vs_Y_EW_dict]
        MW_dicts = [M500_MW_vs_weighted_kT_MW_dict, weighted_kT_MW_vs_M500_MW_dict, M500_MW_vs_Y_MW_dict]
        Xray_dicts = [ weighted_kT_Xray_vs_M500_Xray_dict,  M500_Xray_vs_Lx_in_R500x_dict, weighted_kT_Xray_vs_Lx_in_R500x_dict, M500_Xray_vs_Y_Xray_dict]
        ...
        for k,scaling_dicts in enumerate([Xray_dicts, MW_dicts, EW_dicts]):
            ...
            for i,scaling_dict in enumerate(scaling_dicts):
                ...
                frame = fig.add_axes((.1,.1+ 0.2*i,.8,.2))
                if mass_key == "Xray_HSE_M500":
                    frame.set_xlim(left = np.log10(3e12), right = np.log10(10**13.8))
                else:
                    frame.set_xlim(left = np.log10(3e12), right = np.log10(10**13.8))
                # frame.set_ylim(scaling_power -5, scaling_power+5)
                x_property_key = scaling_dict["x_property_key"]
                ...
                if i%2 == 0:
                    frame.yaxis.set_label_position("left")
                    frame.yaxis.tick_left()
                else:
                    frame.yaxis.set_label_position("right")
                    frame.yaxis.tick_right()
                
                
                print("min mass:", np.array(masses_arr_0))
                print("slope:", np.array(m_arr_0))
                print("slope SD:", np.array(merr_arr_0))
                if i == len(scaling_dicts) - 1:

                    frame.plot(masses_arr_0,np.array(m_arr_0), color = "royalblue", label = "Y|X")
                    plt.fill_between(masses_arr_0,np.array(m_arr_0)+np.array(merr_arr_0),   np.array(m_arr_0)-np.array(merr_arr_0),      alpha = 0.4, color = "dodgerblue")
                    frame.scatter(masses_arr_0,np.array(m_arr_0), color = "royalblue")
                    frame.hlines(y = scaling_power, xmin = 12, xmax = 16, alpha = 0.6, color = "black", ls = "dashed", lw = 2)
                    ...
                    frame.plot(masses_arr_0,np.array(m_arr_0), color = "royalblue")
                    plt.fill_between(masses_arr_0,np.array(m_arr_0)+np.array(merr_arr_0),   np.array(m_arr_0)-np.array(merr_arr_0),      alpha = 0.4, color = "dodgerblue")
                    frame.scatter(masses_arr_0,np.array(m_arr_0), color = "royalblue")
                    frame.hlines(y = scaling_power, xmin = 12, xmax = 16, alpha = 0.6, color = "black", ls = "dashed", lw = 2)
                ...
                if scaling_dict["save_name"] == r"$L_X-M_{500,X}$":
                    frame.set_yticks([0.8,1.3,1.8])  
                    frame.set_yticks([], minor=True)  
                if scaling_dict["save_name"] == r"$Y_X-M_{500,X}$":
                    frame.set_yticks([1.0,1.6,2.2])  
                    frame.set_yticks([], minor=True)                    
                if scaling_dict["save_name"] == r"$M_{500,X}-kT_X$":
                    frame.set_yticks([1.2,1.5,1.8])
                    # frame.set_yticks([1.3,1.4,1.6,1.7],minor = True,)
                    # extraticks = [1.5,]
                    # frame.set_xticks(list(frame.get_xticks()) + extraticks)


                frame.set_ylabel(f"$\mathtt{{{scaling_dict['save_name'].replace(r'$','')}}}$", fontsize = 30)
                try:
    ...
    def _return_binned_means(self, mass_key, best_fit_min_mass, best_fit_max_mass, mass_data, x_data, y_data, x_data_err, y_data_err, best_fit_bins):
        while True:
            err_logx = np.array( [x_data_err[i]/(float(x_data[i])*np.log(10)) for i in range(len(x_data_err))])
            err_logy = np.array( [y_data_err[i]/(float(y_data[i])*np.log(10)) for i in range(len(y_data_err))])
            ### Take the average +/- error as the standard deviation
            ...
            bces_err_logy = np.array(err_logy[sort_idxs])
            mass_range_idxs = np.where( (bces_m_data >= best_fit_min_mass) & (bces_m_data <= best_fit_max_mass) )[0]
            print(f"{len(mass_range_idxs)} out of {len(bces_m_data)} fall in the required mass range ({mass_key} > 10$^{{{np.log10(best_fit_min_mass)}}}$). Min mass = {np.log10(np.min(np.array(bces_m_data[mass_range_idxs]))) }")
            bces_x_data = np.array(bces_x_data[mass_range_idxs])
            bces_y_data = np.array(bces_y_data[mass_range_idxs])
            bces_m_data = np.array(bces_m_data[mass_range_idxs])
            bces_temp_err_logx = np.array(bces_err_logx[mass_range_idxs])
            bces_temp_err_logy = np.array(bces_err_logy[mass_range_idxs])
            
            if np.all(bces_temp_err_logx==0):
                    bces_temp_err_logx = np.ones_like(bces_temp_err_logx) * 0.1
            if np.all(bces_temp_err_logy==0):
                    bces_temp_err_logy = np.ones_like(bces_temp_err_logy) * 0.1

            ### Make the mass bins and assign indexes to the datapoints
            x_range_bins = np.linspace(min(np.log10(bces_x_data)), max(np.log10(bces_x_data)), num = best_fit_bins+1)
            if x_range_bins[-1] > 0: x_range_bins[-1] = 1.01*x_range_bins[-1]  ### Need this else digitize won't handle maximum x-value correctly
            if x_range_bins[-1] < 0: x_range_bins[-1] = 0.99*x_range_bins[-1]
            inds = np.digitize(np.log10(bces_x_data), x_range_bins)
            if max(inds) > best_fit_bins:
                print("Error with bin indices!")
                print("indices:", inds)
                print("bins", x_range_bins)
                return
            if len(set(inds)) < best_fit_bins:
                print(set(inds))
                print("empty bins detected, so reducing number of bins")
            ...
            if 0.0 in [np.sum(np.power(bces_temp_err_logx[inds == i],-2)) for i in range(1,best_fit_bins+1)] or np.inf in [np.sum(np.power(bces_temp_err_logx[inds == i],-2)) for i in range(1,best_fit_bins+1)]:
                print("err_logx", err_logx)
                print("err_logy", err_logy) 
                print(set(inds), f"Best fit bin no = {best_fit_bins}")
                print("bins", x_range_bins)
                print("max x_data", max(np.log10(bces_x_data)))
                print([bces_temp_err_logx[inds == i] for i in range(1,best_fit_bins+1)])
                print([np.power(bces_temp_err_logx[inds == i],-2) for i in range(1,best_fit_bins+1)])
                sys.exit()
            bces_err_logx = np.array([np.sqrt(1/(np.sum(np.power(bces_temp_err_logx[inds == i],-2)))) for i in range(1,best_fit_bins+1)])
            bces_err_logy = np.array([np.sqrt(1/(np.sum(np.power(bces_temp_err_logy[inds == i],-2)))) for i in range(1,best_fit_bins+1)])            

            ### Weight the means
            bces_logx_data = np.array([np.sum(np.log10(bces_x_data[inds == i])/(bces_temp_err_logx[inds == i]**2)) for i in range(1,best_fit_bins+1)])
            bces_logy_data = np.array([np.sum(np.log10(bces_y_data[inds == i])/(bces_temp_err_logy[inds == i]**2)) for i in range(1,best_fit_bins+1)])
            
            ### Correct normalisation for the means 
            bces_logx_data *= bces_err_logx**2
            bces_logy_data *= bces_err_logy**2  
            
            for i in range(1,best_fit_bins+1): 
                if bces_logx_data[i-1] < np.min(np.log10(bces_x_data[inds == i])) and not math.isclose(bces_logx_data[i-1], np.min(np.log10(bces_x_data[inds == i])), rel_tol=1e-3):
                    print("Average is too small!")
                    print(30*"-")
                    print("Data", np.log10(bces_x_data[inds == i]))
                    print("bces_logx_data (mean)", bces_logx_data[i-1])
                    if i != best_fit_bins and len([inds == i]) > 1: sys.exit()
                if bces_logx_data[i-1] > np.max(np.log10(bces_x_data[inds == i])) and not math.isclose(bces_logx_data[i-1], np.max(np.log10(bces_x_data[inds == i])), rel_tol=1e-3):
                    print("Average is too big!")
                    print(30*"-")
                    print("Data", np.log10(bces_x_data[inds == i]))
                    print("bces_logx_data (mean)", bces_logx_data[i-1])
                    if i != best_fit_bins and len([inds == i]) > 1: sys.exit()
            for i in range(1,best_fit_bins+1): 
                if bces_logy_data[i-1] < np.min(np.log10(bces_y_data[inds == i])) and not math.isclose(bces_logy_data[i-1], np.min(np.log10(bces_y_data[inds == i])), rel_tol=1e-3):
                    print("Average is too small!")
                    print(30*"-")
                    print("Data", np.log10(bces_y_data[inds == i]))
                    print("bces_logy_data (mean)", bces_logy_data[i-1])
                    if i != best_fit_bins and len([inds == i]) > 1: sys.exit()
                if bces_logy_data[i-1] > np.max(np.log10(bces_y_data[inds == i])) and not math.isclose(bces_logy_data[i-1], np.max(np.log10(bces_y_data[inds == i])), rel_tol=1e-3):
                    print("Average is too big!")
                    print(30*"-")
                    print("Data", np.log10(bces_y_data[inds == i]))
                    print("bces_logy_data (mean)", bces_logy_data[i-1])
                    if i != best_fit_bins and len([inds == i]) > 1: sys.exit()
                # if len(bces_logx_data) < min_length:
            #     return [None,], None, None, len(bces_logx_data)
            if len(bces_logx_data[np.isnan(bces_logx_data)]) + len(bces_logy_data[np.isnan(bces_logy_data)]) > 0:
                print(bces_logx_data)
                print(bces_logy_data)
                print("Nan values detected, reducing number of bins")
                best_fit_bins -= 1
            else:
                print(f"Binned data from {len(bces_x_data)} points that fell in the mass range into {best_fit_bins} bins")
                return bces_logx_data, bces_logy_data, bces_err_logx, bces_err_logy
