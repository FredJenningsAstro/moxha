...
class Observation:
    ...
    def ObservePhotons(self, image_energies = None, instr_bkgnd = True, foreground = False, ptsrc_bkgnd = True, no_dither = False, overwrite = False, write_foreground_spectrum = False, spectrum_plot_emin = 0.0, spectrum_plot_emax = 3.0, delete_photon_files = False, calibration_markers = False, soxs_stretch = "log", soxs_cmap = "cubehelix", plot_vmin=None, plot_vmax=None, background_suffixes = False, pointing_list = None):
        '''
        ...
        for i,halo in enumerate(self.active_halos):
            ...
            for instrument in self.active_instruments:
                ...
                if background_suffixes:
                    if self._instr_bkgnd:
                        idx_instr_tag += "_instbkg"
                    if self._foreground:
                        idx_instr_tag += "_frg"
                    if self._ptsrc_bkgnd:
                        idx_instr_tag += "_ptsrcbkg"
                ...
                if pointing_list is None:
                    ...
                    obs_list = soxs.make_mosaic_events(pointing_list, f"{self._idx_tag}_halo_simput.fits", f"{evts_path}/{idx_instr_tag}_mosaic", obs_exp_time,
                    ...
                    self._logger.info(f'''instrument_simulator Pars:{self._idx_tag}_halo_mosaic_simput.fits", {evts_path}/{idx_instr_tag}_mosaic_evt.fits", {obs_exp_time}, {instrument_name}, [30., 45.], overwrite=True, instr_bkgnd = {self._instr_bkgnd}, foreground={self._foreground}, ptsrc_bkgnd ={self._ptsrc_bkgnd}, aimpt_shift = {aim_shift}, no_dither = {no_dither}, pointings = {pointing_list}''')
                    self._logger.info(f"Current SOXS Instrument Registry for instr: {soxs.instrument_registry[instrument_name]}")
                    
                    
                    
                    # soxs.write_spectrum(f"{evts_path}/{idx_instr_tag}_mosaic_evt.fits", f"{evts_path}/{idx_instr_tag}_mosaic_evt.pha", overwrite=True)
                    # fig, ax = soxs.plot_spectrum(f"{evts_path}/{idx_instr_tag}_mosaic_evt.pha", xmin=spectrum_plot_emin, xmax=spectrum_plot_emax, lw = 0.5, color = "black" )
                    # fig.set_size_inches((15,5))
                    # plt.xscale("linear")
                    # plt.tight_layout()
                    # # plt.ylim(100,5000000)
    ...
    def _cut_dataset(self):  
        ...
        orig_dset_cuts = copy.deepcopy(self.dataset_cuts)
        for i, cut in enumerate(orig_dset_cuts):
            if 'xray' in cut["field"][1]:
                ...
                if  energies_changed:
                    ...
                    self._logger.info(f"Slightly perturbed the filter emission field from {orig_dset_cuts[i]['field'][1]} to {self.dataset_cuts[i]['field'][1]} in order to not use the yT-generated field instead of the pyXSIM generated field later on")
