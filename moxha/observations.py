...
class Observation:
    ...
    def ObservePhotons(self, image_energies = None, instr_bkgnd = True, foreground = False, ptsrc_bkgnd = True, no_dither = False, overwrite = False, write_foreground_spectrum = False, spectrum_plot_emin = 0.0, spectrum_plot_emax = 3.0, delete_photon_files = False, calibration_markers = False):
        '''
        ...
        for i,halo in enumerate(self.active_halos):
            ...
            for instrument in self.active_instruments:
                ...
                if self._image_energies != None:   
                    for energy_dict in self._image_energies:
                        ...
                        try: 
                            ...
                            fig, ax = soxs.plot_image(f"{evts_path}/{idx_instr_tag}_img_{energy_dict['name']}.fits", stretch='sqrt', cmap='cubehelix', width = instrument['image_width'])
                ...
                if write_foreground_spectrum and foreground:
                    ...
                    fig, ax = soxs.plot_image(f"{evts_path}/{idx_instr_tag}_img_foregrounds.fits", stretch='log', cmap='cubehelix', width = instrument['image_width'])
                    ...
                    if self._image_energies != None:   
                        for energy_dict in self._image_energies:
                            ...
                            try:
                                fig, ax = soxs.plot_image(f"{evts_path}/{idx_instr_tag}_img_{energy_dict['name']}_foregrounds.fits", stretch='log', cmap='cubehelix', width = instrument['image_width'])
                            except:
                                continue
